...
### ⚠️ GAS 플랫폼 한계 (네트워크·런타임) 와 egress-off-GAS 판단

Apps Script 로 프로젝트를 **설계하기 전에** 반드시 점검할 구조적 한계. 코드로 우회 불가능한
것들이라, 요구사항이 여기 걸리면 **아키텍처 자체를 바꿔야** 한다(아래 "탈출구").

**1) 네트워크 (`UrlFetchApp`)**

- egress 가 **구글 공유 IP 풀**을 탄다 → 외부 서비스가 IP 평판/봇 검사를 하면 **차단될 수 있음**.
  실측: Discord webhook 은 GAS IP 가 Cloudflare 에 지속 차단됨(그래서 Slack 으로 대체).
- **프록시·IP 고정 불가** — 차단당해도 IP 를 바꿀 방법이 GAS 안엔 없음.
- 외부 API 에 **캡차 / IP 화이트리스트 / 레이트리밋(429)** 이 있으면 GAS 에서 막히기 쉽다.

**2) 런타임**

- **npm 런타임 패키지 사용 불가** (순수 JS 만). 빌드 과정 없음 → TS/JSX 불가.
- **네이티브 바이너리 / WASM / ONNX / OCR 실행 불가** — 이미지 처리·ML 추론을 GAS 안에서 못 함.
- **실행 6분 제한** (대량 작업은 시간예산 + 이어실행 트리거로 우회), `LockService` 로 동시성 직렬화.

**3) 언제 GAS 로 충분 / 언제 못 버티나**

| GAS 로 충분 ✅          | egress-off-GAS 필요 🚨                  |
| ---------------------- | -------------------------------------- |
| 시트 기반 자동화·소규모 | 외부 API 가 IP평판/캡차/화이트리스트 검사 |
| 평판 안 따지는 공개 API | Discord 등 GAS-차단 서비스 알림         |
| 이미지/ML/OCR 불필요    | ONNX·OCR·네이티브 연산 필요             |
| 간헐적·저빈도 호출      | 대량·저지연·정기 고빈도                 |

**4) 탈출구 — egress-off-GAS 릴레이**

한계에 걸리면 **외부 호출을 GAS 밖 작은 릴레이로 분리**한다. GAS 는 시트+웹UI 로 남고,
릴레이(Cloudflare Workers / Vercel 함수 / Fly.io·Railway·VPS 컨테이너)가 외부 API 를 대신 호출.

- 가벼운 전달(예: Discord 알림) → Workers/Vercel 무료 티어로 충분.
- 무거운 연산(예: 캡차 OCR/ONNX) → Python 가능한 서버리스(Vercel Fluid) 또는 상시 컨테이너.
- 릴레이 IP 도 데이터센터라 또 차단되면 → residential 프록시 추가.

> 실사례: `projects/kingshot-coupon` — Discord→Slack 전환(IP 차단), 공유 IP 429 대응,
> 캡차 도입 대비 컨틴전시(해당 프로젝트 README "컨틴전시: egress-off-GAS" 참조).
