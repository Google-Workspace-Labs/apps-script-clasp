...
function writeGuideSheet_(ss) {
  ...
  sheet
    .getRange('A4:F4')
    .merge()
    .setValue('1️⃣  시트 4개 생성 + Google 권한 승인')
    .setFontSize(13)
    .setFontWeight('bold')
  ...
  sheet
    .getRange('A5:F5')
    .merge()
    .setValue('① 상단 메뉴 [👑 Kingshot Bot ▸ 🚀 Setup ▸ Setup Sheets] 클릭')
    .setFontSize(11);
  sheet
    .getRange('A6:F6')
    .merge()
    .setValue('② Google 권한 승인 — "확인하지 않은 앱" 경고가 떠도 정상입니다:')
    .setFontSize(11);
  sheet
    .getRange('A7:F7')
    .merge()
    .setValue("     ⓐ 좌측 하단 [고급] 클릭 ('안전한 환경으로 돌아가기' 누르지 말 것)")
    .setFontSize(10)
    .setFontColor('#555');
  sheet
    .getRange('A8:F8')
    .merge()
    .setValue('     ⓑ 펼쳐진 하단의 [Kingshot Coupon(으)로 이동(안전하지 않음)] 클릭')
    .setFontSize(10)
    .setFontColor('#555');
  sheet
    .getRange('A9:F9')
    .merge()
    .setValue('     ⓒ 권한 목록에서 [모두 선택] 체크 → [계속] 클릭')
    .setFontSize(10)
    .setFontColor('#555');
  sheet
    .getRange('A10:F10')
    .merge()
    .setValue('     ※ "안전하지 않음" 표시는 Google 미인증 안내일 뿐 — 코드는 안전합니다')
    .setFontSize(10)
    .setFontColor('#888')
    .setFontStyle('italic');
  sheet
    .getRange('A11:F11')
    .merge()
    .setValue('③ 시트 4개(users · coupons · logs · system_logs) 가 자동 생성됩니다')
    .setFontSize(11);
  sheet
    .getRange('A12:F12')
    .merge()
    .setValue('✅ 위 4개 시트가 이미 보이면 1단계는 완료된 상태입니다')
    .setFontSize(10)
    .setFontColor('#888')
    .setFontStyle('italic');
  ...
  sheet
    ...
    .merge()
    .setValue(
      '① [👑 Kingshot Bot ▸ 🚀 Setup ▸ Quick Setup] 클릭 → 모달의 [▶ Apps Script 에디터 열기] 버튼',
    )
    .setFontSize(11);
  sheet
    .getRange('A16:F16')
    .merge()
    .setValue('② 에디터 우측 상단 [배포 ▸ 새 배포] → 좌측 [⚙️ 유형] → [웹 앱] 선택')
    .setFontSize(11);
  sheet
    .getRange('A17:F17')
    .merge()
    .setValue('③ 구성: 실행 사용자 = 나(본인), 액세스 권한 = 모든 사용자 → [배포] 클릭')
    .setFontSize(11);
  sheet
    .getRange('A18:F18')
    .merge()
    .setValue(
      '④ "액세스 권한 부여 요청" 화면 → [액세스 승인] → 1단계의 ⓐⓑⓒ 와 동일하게 권한 모두 허용',
    )
    .setFontSize(11);
  sheet
    .getRange('A19:F19')
    .merge()
    .setValue('⑤ "배포가 업데이트되었습니다" 화면 → 웹 앱 URL 의 [복사] 버튼 → [완료]')
    .setFontSize(11);
  sheet
    .getRange('A20:F20')
    .merge()
    .setValue('⑥ 복사한 /exec URL 을 브라우저로 접속 확인')
    .setFontSize(11);
  sheet
    .getRange('A21:F21')
    .merge()
